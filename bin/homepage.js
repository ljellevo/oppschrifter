...
Main.main = function() {
	...
	auth(function(response2) {
		if(response2 == "true") {
			...
			com_vige_core_Navigate.to(null,{ url : window.location.pathname, main : true});
...
classes_Recipe.prototype = {
	constr: function(name,category,hasLink,ingredients,amounts,steps,url,tags,id,uploaded,viewed) {
		this.id = id;
		this.name = name;
	...
	,toJSON: function() {
		var object = { name : this.name, category : this.category, hasLink : this.hasLink, ingredients : this.ingredients, amounts : this.amounts, steps : this.steps, url : this.url, tags : this.tags, dbTags : this.tags + " " + this.name + " " + this.category, uploaded : this.uploaded, viewed : this.viewed};
...
var com_vige_components_Button = function(arg) {
	...
	this.child = null;
	this.child = arg.child;
	this.isLink = arg.isLink != null && arg.isLink;
	this.onClick = arg.onClick;
	this.color = arg.color;
	this.border = arg.border;
	this.padding = arg.padding;
...
com_vige_components_Button.prototype = {
	...
	,render: function() {
		if(this.isLink) {
			...
			new com_vige_support_StyleManager().addStyleToAnchor({ widget : button, color : this.color, border : this.border, padding : this.padding, margin : this.margin, size : this.size, overflow : this.overflow, shadow : this.shadow});
			return button;
		} else {
			var button1 = window.document.createElement("button");
			button1.appendChild(this.child.render());
			...
			new com_vige_support_StyleManager().addStyleToButton({ widget : button1, color : this.color, border : this.border, padding : this.padding, margin : this.margin, size : this.size, overflow : this.overflow, shadow : this.shadow});
			return button1;
		}
	}
...
var com_vige_components_InputController = function() {
};
com_vige_components_InputController.__name__ = "com.vige.components.InputController";
com_vige_components_InputController.prototype = {
	getElement: function() {
		return this.element;
	}
	...
	,__class__: com_vige_components_InputController
};
var com_vige_components_Input = function(arg) {
	this.required = false;
	this.readOnly = false;
	this.pattern = "(.*?)";
	...
	this.autofocus = arg.autofocus != null && arg.autofocus;
	this.pattern = arg.pattern != null ? arg.pattern : "(.*?)";
	this.readOnly = arg.readOnly != null && arg.readOnly;
	this.required = arg.required != null && arg.required;
...
com_vige_components_Input.__name__ = "com.vige.components.Input";
com_vige_components_Input.__interfaces__ = [com_vige_support_Widget];
com_vige_components_Input.prototype = {
	init: function() {
	}
	,render: function() {
		...
		new com_vige_support_StyleManager().addStyleToInput({ widget : input, color : this.color, border : this.border, padding : this.padding, margin : this.margin, size : this.size, overflow : this.overflow});
		this.controller.setElement(input);
		return input;
	}
	,__class__: com_vige_components_Input
};
...
var com_vige_support_InputType = $hxEnums["com.vige.support.InputType"] = { __ename__ : true, __constructs__ : ["Button","Checkbox","Color","Date","DatetimeLocal","Email","File","Hidden","Image","Month","Number","Password","Radio","Range","Reset","Search","Submit","Tel","Text","Time","Url","Week"]
	...
	,Date: {_hx_index:3,__enum__:"com.vige.support.InputType",toString:$estr}
...
com_vige_support_StyleManager.prototype = {
	addStyleToDiv: function(arg) {
		...
		if(arg.shadow != null && arg.shadow.length > 0) {
			haxe_Log.trace("Shadow was found",{ fileName : "com/vige/support/StyleManager.hx", lineNumber : 174, className : "com.vige.support.StyleManager", methodName : "addStyleToDiv"});
			var joinedString = "";
			var _g = 0;
			...
			haxe_Log.trace("Box-shadow: " + joinedString,{ fileName : "com/vige/support/StyleManager.hx", lineNumber : 183, className : "com.vige.support.StyleManager", methodName : "addStyleToDiv"});
...
var components_CustomNavbar = function() {
	com_vige_core_DynamicComponent.call(this);
};
components_CustomNavbar.__name__ = "components.CustomNavbar";
components_CustomNavbar.__super__ = com_vige_core_DynamicComponent;
components_CustomNavbar.prototype = $extend(com_vige_core_DynamicComponent.prototype,{
	homepageButton: function(text,src,url) {
		...
		var getButtonContents = function(text1,src1) {
			...
			if(src1 != null && src1 != "") {
				if(url == "/") {
					widgets.push(new com_vige_components_Container({ size : new com_vige_utils_Size({ width : null}), color : new com_vige_utils_Color({ color : -16777216}), child : new com_vige_components_Image({ src : src1, height : 25, resizeModifier : com_vige_support_ResizeModifier.Height})}));
				} else {
					widgets.push(new com_vige_components_Container({ size : new com_vige_utils_Size({ width : null}), color : new com_vige_utils_Color({ color : -16777216}), child : new com_vige_components_Image({ src : src1, height : 15, resizeModifier : com_vige_support_ResizeModifier.Height})}));
				}
			}
			if(text1 != null && text1 != "") {
				if(src1 != null && src1 != "") {
					widgets.push(new com_vige_components_Container({ size : new com_vige_utils_Size({ width : "20px"})}));
				}
				var this1 = Std.parseInt("0xff" + HxOverrides.substr("#2e3440",1,null));
				widgets.push(new com_vige_components_Text(text1,{ textSize : 12, color : new com_vige_utils_Color({ color : this1})}));
			}
			return widgets;
		};
		...
		var tmp = new com_vige_utils_Size({ height : "30px", width : "70px"});
		...
		return new com_vige_components_Button({ size : tmp, color : new com_vige_utils_Color({ color : -16777216, backgroundColor : this3}), border : determineBorder(), child : new com_vige_components_Row({ mainAxisAlignment : com_vige_support_MainAxisAlignment.Center, children : getButtonContents(text,src)}), isLink : false, onClick : function(e) {
			...
			com_vige_core_Navigate.to(_gthis,{ url : url});
		}});
	}
	,logoutButton: function(text,src,url) {
		var getButtonContents = function(text1,src1) {
			var widgets = [];
			if(src1 != null && src1 != "") {
				if(url == "/") {
	...
	,navbarComponent: function() {
		var this1 = Std.parseInt("0xff" + HxOverrides.substr("#fafafa",1,null));
		var navbar = new com_vige_utils_Color({ backgroundColor : this1});
		var navbar1 = new com_vige_components_Container({ color : new com_vige_utils_Color({ backgroundColor : -16711681}), size : new com_vige_utils_Size({ width : "100%", height : "20px"})});
		var navbar2 = new com_vige_components_Container({ color : new com_vige_utils_Color({ backgroundColor : -65536}), size : new com_vige_utils_Size({ width : "100%", height : "20px"})});
		var navbar3 = new com_vige_components_Container({ color : new com_vige_utils_Color({ backgroundColor : -16776961}), size : new com_vige_utils_Size({ width : "100%", height : "20px"})});
		var this2 = Std.parseInt("0xff" + HxOverrides.substr("#CDCDCD",1,null));
		var navbar4 = new com_vige_components_Navbar({ position : com_vige_support_NavbarPosition.Top, offset : 0, color : navbar, child : new com_vige_components_Column({ children : [navbar1,navbar2,navbar3,new com_vige_components_Row({ shadow : [new com_vige_utils_Shadow({ horizontal : "0", vertical : "8px", blur : "6px", spread : "-6px", color : new com_vige_utils_Color({ backgroundColor : this2})})], mainAxisAlignment : com_vige_support_MainAxisAlignment.Center, crossAxisAlignment : com_vige_support_CrossAxisAlignment.SpacedEvenly, margin : com_vige_utils_Margin.fromTRBL(10,0,10,0), children : [this.homepageButton("Oversikt",null,"/"),this.homepageButton("Ny",null,"/add"),this.homepageButton("Feedback",null,"/feedback"),this.logoutButton("Logg ut",null,"/logout")]})]}), onComplete : function() {
		}});
		return navbar4;
	}
	,__class__: components_CustomNavbar
});
var components_DateConverter = function() { };
components_DateConverter.__name__ = "components.DateConverter";
components_DateConverter.convertTimestampToString = function(timestamp) {
...
var pages_AddPage = function() {
	this.stepAmount = 1;
	this.stepControllersIntermediateValues = [];
	this.stepControllers = [];
	this.stepInputController = new com_vige_components_InputController();
	this.amountControllersIntermediateValues = [];
	this.amountController = [];
	this.amountInputController = new com_vige_components_InputController();
	this.ingredientControllersIntermediateValues = [];
	this.ingredientController = [];
	this.ingredientInputController = new com_vige_components_InputController();
	this.ingredientAmount = 1;
	this.tagsValue = "";
	this.tagsInputController = new com_vige_components_InputController();
	this.urlValue = "";
	this.urlInputController = new com_vige_components_InputController();
	this.linkInputController = new com_vige_components_InputController();
	this.categoryValue = "";
	this.categoryInputController = new com_vige_components_InputController();
	this.nameValue = "";
	this.nameInputController = new com_vige_components_InputController();
	this.isLink = false;
	this.status = "";
	this.data = [];
	com_vige_core_DynamicComponent.call(this);
};
pages_AddPage.__name__ = "pages.AddPage";
pages_AddPage.__super__ = com_vige_core_DynamicComponent;
pages_AddPage.prototype = $extend(com_vige_core_DynamicComponent.prototype,{
	addRecipe: function() {
		...
		testFunc(this.newRecipe.toJSON(),function(response1) {
			com_vige_core_Navigate.to(_gthis,{ url : "/", hardRefresh : true});
		});
	}
	,generateIngredients: function() {
		var widgets = [];
		var _g = 0;
		var _g1 = this.ingredientAmount;
		while(_g < _g1) {
			var i = _g++;
			var ingredientTempController = new com_vige_components_InputController();
			var amountTempController = new com_vige_components_InputController();
			var ingredientValue = "";
			var amountValue = "";
			if(i >= this.ingredientController.length) {
				this.ingredientController.push(ingredientTempController);
				this.amountController.push(amountTempController);
				this.ingredientControllersIntermediateValues.push(ingredientValue);
				this.amountControllersIntermediateValues.push(amountValue);
			} else {
				ingredientTempController = this.ingredientController[i];
				amountTempController = this.amountController[i];
				ingredientValue = this.ingredientControllersIntermediateValues[i];
				amountValue = this.amountControllersIntermediateValues[i];
			}
			widgets.push(new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Row({ size : new com_vige_utils_Size({ width : "75%", maxWidth : "450px"}), children : [new com_vige_components_Input({ type : com_vige_support_InputType.Search, controller : ingredientTempController, placeholder : "Ingrediens", value : ingredientValue}),new com_vige_components_Container({ size : new com_vige_utils_Size({ width : "20px"})}),new com_vige_components_Input({ type : com_vige_support_InputType.Search, controller : amountTempController, placeholder : "Mengde", value : amountValue})]})})}));
		}
		return widgets;
	}
	,generateSteps: function() {
		var widgets = [];
		var _g = 0;
		var _g1 = this.stepAmount;
		while(_g < _g1) {
			var i = _g++;
			var controller = new com_vige_components_InputController();
			var value = "";
			if(i >= this.stepControllers.length) {
				this.stepControllers.push(controller);
				this.stepControllersIntermediateValues.push(value);
			} else {
				controller = this.stepControllers[i];
				value = this.stepControllersIntermediateValues[i];
			}
			widgets.push(new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Input({ type : com_vige_support_InputType.Search, controller : controller, placeholder : "Steg " + (i + 1), value : value, size : new com_vige_utils_Size({ width : "75%", maxWidth : "450px"})})})}));
		}
		return widgets;
	}
	,getCorrectSection: function() {
		var _gthis = this;
		if(this.isLink) {
			return new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Input({ type : com_vige_support_InputType.Search, controller : this.urlInputController, placeholder : "Link til oppskrift", value : this.urlValue, size : new com_vige_utils_Size({ width : "75%", maxWidth : "450px"})})})});
		} else {
			var tmp = com_vige_utils_Padding.fromTRBL(30,0,30,0);
			var tmp1 = new com_vige_support_Fonts("Poppins","sans-serif");
			var this1 = Std.parseInt("0xff" + HxOverrides.substr("#2e3440",1,null));
			var tmp2 = new com_vige_components_Container({ padding : tmp, child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Text("Ingredienser",{ textSize : 20, font : tmp1, fontWeight : com_vige_support_FontWeight.W900, color : new com_vige_utils_Color({ color : this1})})})});
			var tmp3 = new com_vige_components_Column({ children : this.generateIngredients()});
			var tmp4 = new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Button({ child : new com_vige_components_Text("+"), onClick : function() {
				_gthis.saveValues();
				_gthis.setState(_gthis,function(e) {
					_gthis.ingredientAmount++;
				});
			}})})});
	...
	,saveValues: function() {
		this.nameValue = this.nameInputController.getValue();
		this.categoryValue = this.categoryInputController.getValue();
		this.urlValue = this.urlInputController.getValue();
		this.tagsValue = this.tagsInputController.getValue();
		this.linkValue = this.linkInputController.getElement().checked;
		var _g = 0;
		var _g1 = this.ingredientController.length;
		while(_g < _g1) {
			var i = _g++;
			this.ingredientControllersIntermediateValues[i] = this.ingredientController[i].getValue();
			this.amountControllersIntermediateValues[i] = this.amountController[i].getValue();
		}
		var _g2 = 0;
		var _g3 = this.stepControllers.length;
		while(_g2 < _g3) {
			var i1 = _g2++;
			this.stepControllersIntermediateValues[i1] = this.stepControllers[i1].getValue();
		}
	}
	,component: function() {
		var _gthis = this;
		var tmp = new components_CustomNavbar().navbarComponent();
		var tmp1 = com_vige_utils_Padding.fromTRBL(30,0,30,0);
		var tmp2 = new com_vige_support_Fonts("Poppins","sans-serif");
		var this1 = Std.parseInt("0xff" + HxOverrides.substr("#2e3440",1,null));
		this.page = new com_vige_components_Page({ navbar : tmp, route : "/add", child : new com_vige_components_Column({ children : [new com_vige_components_Container({ padding : tmp1, child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Text("NY",{ textSize : 40, font : tmp2, fontWeight : com_vige_support_FontWeight.W900, color : new com_vige_utils_Color({ color : this1})})})}),new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Input({ type : com_vige_support_InputType.Search, controller : this.nameInputController, placeholder : "Navn på oppskrift", value : this.nameValue, size : new com_vige_utils_Size({ width : "75%", maxWidth : "450px"})})})}),new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Input({ type : com_vige_support_InputType.Search, controller : this.categoryInputController, placeholder : "Kategori", value : this.categoryValue, size : new com_vige_utils_Size({ width : "75%", maxWidth : "450px"})})})}),new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Row({ size : new com_vige_utils_Size({ width : "75%", maxWidth : "450px"}), children : [new com_vige_components_Text("Vil du legge til en link?",{ size : new com_vige_utils_Size({ width : "10px"})}),new com_vige_components_Input({ type : com_vige_support_InputType.Checkbox, margin : com_vige_utils_Margin.fromTRBL(0,0,0,10), controller : this.linkInputController, placeholder : "Stikkord", size : new com_vige_utils_Size({ width : "10px"}), checked : this.linkValue, onClick : function() {
			_gthis.linkValue = _gthis.linkInputController.getElement().checked;
		...
		}})], mainAxisAlignment : com_vige_support_MainAxisAlignment.Left})})}),this.getCorrectSection(),new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Input({ type : com_vige_support_InputType.Search, controller : this.tagsInputController, placeholder : "Stikkord", value : this.tagsValue, size : new com_vige_utils_Size({ width : "75%", maxWidth : "450px"})})})}),new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Button({ child : new com_vige_components_Text("Legg til"), onClick : function() {
			_gthis.saveValues();
			_gthis.newRecipe = new classes_Recipe();
			_gthis.newRecipe.constr(_gthis.nameInputController.getValue(),_gthis.categoryInputController.getValue(),_gthis.isLink,_gthis.ingredientControllersIntermediateValues,_gthis.amountControllersIntermediateValues,_gthis.stepControllersIntermediateValues,_gthis.urlInputController.getValue(),_gthis.tagsInputController.getValue());
			_gthis.addRecipe();
		}})})})]})});
		return this.page;
	}
	,__class__: pages_AddPage
});
var pages_HomePage = function() {
	this.searchInputController = new com_vige_components_InputController();
	this.data = [];
	com_vige_core_DynamicComponent.call(this);
	this.getRecipe(function(token) {
		haxe_Log.trace("Login was successfull and token was recived",{ fileName : "src/pages/HomePage.hx", lineNumber : 30, className : "pages.HomePage", methodName : "new"});
	});
};
...
pages_HomePage.prototype = $extend(com_vige_core_DynamicComponent.prototype,{
		...
		new com_vige_core_SingleRequest({ url : Config.API_RECIPE + this.searchInputController.getValue(), method : "GET", onComplete : function(res) {
			_gthis.setState(_gthis,function() {
				...
				while(_g < _g1) {
					...
					newRecipe.constr(value[i].name,value[i].category,value[i].hasLink,value[i].ingredients,value[i].amounts,value[i].steps,value[i].url,value[i].tags,value[i]._id,value[i].uploaded,value[i].viewed);
	...
	,component: function() {
		...
		window.addEventListener("keypress",function(e) {
			if(e.keyCode == 13) {
				...
				_gthis.getRecipe(function(token) {
					haxe_Log.trace("Login was successfull and token was recived",{ fileName : "src/pages/HomePage.hx", lineNumber : 80, className : "pages.HomePage", methodName : "component"});
				});
			}
		});
		var tmp = new components_CustomNavbar().navbarComponent();
		...
		var tmp2 = new com_vige_support_Fonts("Poppins","sans-serif");
		var this1 = Std.parseInt("0xff" + HxOverrides.substr("#2e3440",1,null));
		this.page = new com_vige_components_Page({ navbar : tmp, route : "/", child : new com_vige_components_Column({ children : [new com_vige_components_Container({ padding : tmp1, child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Text("OPPSCHRIFTER",{ textSize : 40, font : tmp2, fontWeight : com_vige_support_FontWeight.W900, color : new com_vige_utils_Color({ color : this1})})})}),new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Input({ type : com_vige_support_InputType.Search, controller : this.searchInputController, placeholder : "Søk etter oppskrift", size : new com_vige_utils_Size({ width : "75%", maxWidth : "450px"}), value : this.currentValue, onChange : function() {
			...
			haxe_Log.trace("Search was clicked",{ fileName : "src/pages/HomePage.hx", lineNumber : 138, className : "pages.HomePage", methodName : "component"});
			_gthis.currentValue = _gthis.searchInputController.getValue();
			...
			var tmp10 = new com_vige_components_Row({ children : [new com_vige_components_Text(tmp9,{ color : new com_vige_utils_Color({ color : this5}), textSize : 18}),new com_vige_components_Container({ child : new com_vige_components_Text(components_DateConverter.convertTimestampToString(_gthis.data[iterator].getUploaded()),{ textAlignment : com_vige_support_TextAlign.Right, textSize : 12}), size : new com_vige_utils_Size({ width : "110px"})})], flex : true, mainAxisAlignment : com_vige_support_MainAxisAlignment.Stretch, crossAxisAlignment : com_vige_support_CrossAxisAlignment.SpaceBetween, equalElementWidth : false});
			var tmp11 = _gthis.data[iterator].getCategory();
			var this6 = Std.parseInt("0xff" + HxOverrides.substr("#A4A4A4",1,null));
			return new com_vige_components_Action({ onClick : function() {
				var tmp12 = "/recipe/" + _gthis.data[iterator].getId();
				com_vige_core_Navigate.to(_gthis,{ url : tmp12});
			}, child : new com_vige_components_Container({ color : tmp3, margin : tmp4, size : tmp5, shadow : tmp7, padding : tmp8, child : new com_vige_components_Column({ children : [tmp10,new com_vige_components_Text(tmp11,{ color : new com_vige_utils_Color({ color : this6}), textSize : 12})]})})});
...
var pages_LoginPage = function() {
	this.passwordInputController = new com_vige_components_InputController();
	this.usernameInputController = new com_vige_components_InputController();
	this.errorMessage = "";
	this.data = ["Pasta","Pizza"];
	com_vige_core_DynamicComponent.call(this);
};
pages_LoginPage.__name__ = "pages.LoginPage";
pages_LoginPage.__super__ = com_vige_core_DynamicComponent;
pages_LoginPage.prototype = $extend(com_vige_core_DynamicComponent.prototype,{
	login: function(callback) {
		...
		var request = function(username,password,callback1) {
			var sha = haxe_crypto_Sha256.encode(password);
			object = { username : username, password : sha};
			req.setHeader("Content-type","application/json");
			var request1 = JSON.stringify(object);
			req.setPostData(request1);
			req.onData = function(response) {
				callback1(response);
			};
		...
		request(this.usernameInputController.getValue(),this.passwordInputController.getValue(),function(response1) {
			...
			com_vige_core_Navigate.to(_gthis,{ url : "/"});
			window.location.reload();
		});
	}
	,infoLable: function() {
		if(this.errorMessage == "Http Error #400") {
			return new com_vige_components_Text("Brukernavn eller passord er feil");
		} else if(this.errorMessage != "") {
			return new com_vige_components_Text(this.errorMessage);
		}
		return new com_vige_components_Container({ });
	}
	,component: function() {
		var _gthis = this;
		var tmp = new com_vige_components_Container({ color : new com_vige_utils_Color({ backgroundColor : -16711681}), size : new com_vige_utils_Size({ width : "100%", height : "20px"})});
		...
		this.page = new com_vige_components_Page({ route : "/", child : new com_vige_components_Column({ children : [tmp,tmp1,tmp2,new com_vige_components_Center({ margin : tmp3, alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Container({ shadow : tmp5, color : tmp6, size : tmp7, child : new com_vige_components_Column({ children : [new com_vige_components_Container({ padding : tmp8, child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Text("LOGG INN",{ textSize : 40, font : tmp9, fontWeight : com_vige_support_FontWeight.W900, color : new com_vige_utils_Color({ color : this4})})})}),this.infoLable(),new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Input({ type : com_vige_support_InputType.Search, controller : this.usernameInputController, placeholder : "Brukernavn", size : new com_vige_utils_Size({ width : "50%", maxWidth : "250px"})})})}),new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Input({ type : com_vige_support_InputType.Password, controller : this.passwordInputController, placeholder : "Passord", size : new com_vige_utils_Size({ width : "50%", maxWidth : "250px"})})})}),new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Row({ children : [new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Button({ child : new com_vige_components_Text("Register"), onClick : function() {
			...
			com_vige_core_Navigate.to(_gthis,{ url : "/register"});
...
pages_RecipePage.prototype = $extend(com_vige_core_DynamicComponent.prototype,{
	...
	,unload: function() {
		haxe_Log.trace("unloaded",{ fileName : "src/pages/RecipePage.hx", lineNumber : 33, className : "pages.RecipePage", methodName : "unload"});
		this.page = new com_vige_components_Page({ route : "/recipe/:id", child : new com_vige_components_Container({ })});
		this.component();
	}
	,getRecipe: function() {
		var _gthis = this;
		new com_vige_core_SingleRequest({ url : Config.API_RECIPE_GET + com_vige_core_Navigate.getParameters()[0], method : "GET", onComplete : function(res) {
			_gthis.setState(_gthis,function() {
				...
				recipe.constr(value.name,value.category,value.hasLink,value.ingredients,value.amounts,value.steps,value.url,value.tags,value._id,value.uploaded,value.viewed);
				_gthis.data = recipe;
			});
		}, onProgress : function() {
		}, onError : function(error) {
	...
	,isLink: function() {
		var _gthis = this;
		if(this.data.getHasLink()) {
			var tmp = com_vige_utils_Padding.fromTRBL(0,0,30,0);
			var tmp1 = new com_vige_utils_Size({ width : "75%", maxWidth : "450px"});
			var this1 = Std.parseInt("0xff" + HxOverrides.substr("#fafafa",1,null));
			var tmp2 = new com_vige_utils_Color({ backgroundColor : this1});
			var this2 = Std.parseInt("0xff" + HxOverrides.substr("#CDCDCD",1,null));
			var tmp3 = new com_vige_utils_Shadow({ horizontal : "0px", vertical : "4px", blur : "6px", color : new com_vige_utils_Color({ backgroundColor : this2})});
			var this3 = Std.parseInt("0xff" + HxOverrides.substr("#CDCDCD",1,null));
			var tmp4 = [tmp3,new com_vige_utils_Shadow({ horizontal : "0px", vertical : "0px", blur : "2px", color : new com_vige_utils_Color({ backgroundColor : this3})})];
			var tmp5 = com_vige_utils_Padding.all(20);
			var this4 = Std.parseInt("0xff" + HxOverrides.substr("#2e3440",1,null));
			return new com_vige_components_Container({ padding : tmp, child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Column({ size : tmp1, children : [new com_vige_components_Action({ onClick : function() {
				com_vige_core_Navigate.link({ url : _gthis.data.getUrl()});
			}, child : new com_vige_components_Container({ color : tmp2, shadow : tmp4, padding : tmp5, child : new com_vige_components_Row({ children : [new com_vige_components_Text("KLIKK HER",{ color : new com_vige_utils_Color({ color : this4}), textAlignment : com_vige_support_TextAlign.Center, textSize : 20})]})})})]})})});
		}
		return new com_vige_components_Container({ });
	}
	,generateSteps: function() {
		...
		var tmp = new com_vige_components_Text("Ingredienser",{ color : new com_vige_utils_Color({ color : this1}), textSize : 12});
		var tmp1 = new com_vige_components_Column({ children : com_vige_components_Constructors.constructRows({ data : this.data.getIngredients(), elementsInEachRow : 1, elementBuilder : function(i) {
			return new com_vige_components_Text(" - " + _gthis.data.getIngredients()[i] + " " + _gthis.data.getAmounts()[i],{ textSize : 12});
		}, rowBuilder : function(children) {
			return new com_vige_components_Row({ children : children});
		}})});
		...
		return new com_vige_components_Container({ child : new com_vige_components_Column({ children : [tmp,tmp1,tmp2,new com_vige_components_Text("Fremgangsmåte",{ color : new com_vige_utils_Color({ color : this2}), textSize : 12}),new com_vige_components_Column({ children : com_vige_components_Constructors.constructRows({ data : this.data.getSteps(), elementsInEachRow : 1, elementBuilder : function(i1) {
			return new com_vige_components_Text(Std.string(i1 + 1) + ". " + _gthis.data.getSteps()[i1],{ textSize : 12});
	...
	,component: function() {
		...
		if(this.data != null) {
			...
			var tmp9 = new com_vige_support_Fonts("Poppins","sans-serif");
			var this4 = Std.parseInt("0xff" + HxOverrides.substr("#2e3440",1,null));
			var tmp10 = new com_vige_components_Container({ padding : tmp7, child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Text(tmp8,{ textSize : 20, font : tmp9, fontWeight : com_vige_support_FontWeight.W900, color : new com_vige_utils_Color({ color : this4})})})});
			...
			var tmp13 = new com_vige_components_Text(tmp12,{ color : new com_vige_utils_Color({ color : this5}), textSize : 12});
			var tmp14 = Std.string(this.data.getViewed() + " visninger");
			var this6 = Std.parseInt("0xff" + HxOverrides.substr("#A4A4A4",1,null));
			var tmp15 = new com_vige_components_Text(tmp14,{ color : new com_vige_utils_Color({ color : this6}), textSize : 12});
			...
			this.page = new com_vige_components_Page({ navbar : new components_CustomNavbar().navbarComponent(), route : "/recipe/:id", child : new com_vige_components_Column({ children : []})});
			return this.page;
		}
	}
	,__class__: pages_RecipePage
});
var pages_RegisterPage = function() {
	this.retypePasswordInputController = new com_vige_components_InputController();
	this.passwordInputController = new com_vige_components_InputController();
	this.usernameInputController = new com_vige_components_InputController();
	this.errorMessage = "";
	this.data = ["Pasta","Pizza"];
...
pages_RegisterPage.__name__ = "pages.RegisterPage";
pages_RegisterPage.__super__ = com_vige_core_DynamicComponent;
pages_RegisterPage.prototype = $extend(com_vige_core_DynamicComponent.prototype,{
	login: function(callback) {
		var _gthis = this;
		var object;
		...
		if(this.passwordInputController.getValue() == "") {
			return;
		}
		if(this.passwordInputController.getValue() != this.retypePasswordInputController.getValue()) {
			return;
		}
	...
	,component: function() {
		...
		this.page = new com_vige_components_Page({ route : "/register", child : new com_vige_components_Column({ children : [tmp,tmp1,tmp2,new com_vige_components_Center({ margin : tmp3, alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Container({ shadow : tmp4, color : tmp5, size : tmp6, child : new com_vige_components_Column({ children : [new com_vige_components_Container({ padding : tmp7, child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Text("NY BRUKER",{ textSize : 40, font : tmp8, fontWeight : com_vige_support_FontWeight.W900, color : new com_vige_utils_Color({ color : this3})})})}),this.infoLable(),new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Input({ type : com_vige_support_InputType.Search, controller : this.usernameInputController, placeholder : "Brukernavn", size : new com_vige_utils_Size({ width : "50%", maxWidth : "250px"})})})}),new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Input({ type : com_vige_support_InputType.Password, controller : this.passwordInputController, placeholder : "Passord", size : new com_vige_utils_Size({ width : "50%", maxWidth : "250px"})})})}),new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Input({ type : com_vige_support_InputType.Password, controller : this.retypePasswordInputController, placeholder : "Gjenta Passord", size : new com_vige_utils_Size({ width : "50%", maxWidth : "250px"})})})}),new com_vige_components_Container({ padding : com_vige_utils_Padding.fromTRBL(0,0,30,0), child : new com_vige_components_Row({ children : [new com_vige_components_Center({ alignment : com_vige_support_CenterAlignment.Both, child : new com_vige_components_Button({ child : new com_vige_components_Text("Allerede bruker?"), onClick : function() {
			...
			com_vige_core_Navigate.to(_gthis,{ url : "/login"});
...
Config.API_AUTH = "/api/auth";
Config.API_REGISTER = "/api/register";
Config.API_LOGIN = "/api/login";
Config.API_RECIPE_GET = "/api/recipe/";
Config.API_RECIPE_DELETE = "/api/recipe/";
Config.API_RECIPE = "/api/recipes/";
Config.API_RECIP_POST = "/api/recipe";
Xml.Element = 0;
Xml.PCData = 1;
Xml.CData = 2;
Xml.Comment = 3;
